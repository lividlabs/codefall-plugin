...
| --- | --- |
| Inherited ADRs — new, revised, renamed | Moving code between architectures or topologies |
...
| --- | --- | --- |
| **untouched** | hash matches the recorded one, `amended: false` | replace or rename it when the user takes the item |
...
**Only untouched is ever safe to update automatically.** An amended or edited ADR is a decision
this project made on purpose; overwriting it with the current template would replace a ratified
decision with a default. The diff *is* the deliverable for those two — merging it is the user's
work, and after they merge they can run graft again.

There is one narrow exception: a user who has seen the diff may explicitly say *discard my version
and take the template*. That is their file and their call — but it discards a decision the project
ratified, so say exactly that, require them to name the file, and only then treat it as a fresh
instantiation. Never offer this as the convenient path.
...
automatically or is the user's to merge.
...
Only requested items, and only missing or untouched ones are ever written:
...
- **Missing** — instantiate from the current template, exactly as `scaffold` step 4 emits it:
  ...
  layout.
- **Untouched, revised** — replace the body with the current template and stamp today's date; the
  project is re-ratifying the revised decision, and the decision log records when the original was
  adopted.
- **Renamed** — move the file (`git mv` where the tree is git-managed). Fix references to the old
  identifier in *untouched* inherited docs as part of the move. Stale references in amended or
  edited docs, in `AGENTS.md`, or in the project's own ADRs are theirs: list each one, offer the
  targeted edit, and make it only on a yes.
- **Amended / edited** — never overwritten by taking an update, even one that renames the file
...
- **`.codefall/scaffold.json`** — after every applied item, update its entry: `id`, `file`, and a
  `sha256` computed with `shasum -a 256` on the file as written — never invented. Add
...
IDs and check the hits against that list. Then show the git diff summary: the graft should be
